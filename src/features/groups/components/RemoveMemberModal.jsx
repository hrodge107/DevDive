export default function RemoveMemberModal({ member, isOwner, onConfirm, onCancel }) {
  const username = member?.profiles?.username || member?.profiles?.email || 'Unknown';
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity">
        <div className="bg-gray-900 border border-gray-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-5 border-b border-gray-800 bg-gray-900/50">
            <h3 className="text-lg font-bold text-amber-500 tracking-tight flex items-center gap-2">
          ...
          <div className="p-6 space-y-6">
            <p className="text-gray-300 leading-relaxed text-sm">
              Owners cannot remove themselves from the group.
            </p>
            <div className="flex justify-end pt-2">
              <button 
                type="button" 
                onClick={onCancel} 
                className="px-4 py-2.5 bg-gray-800 hover:bg-gray-700 text-white rounded-lg transition-colors text-sm font-semibold"
              >
                Close
              </button>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity">
      <div className="bg-gray-900 border border-gray-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6">
          <p className="text-gray-300 leading-relaxed text-sm">
            Are you sure you want to remove <strong className="text-white font-semibold">"{username}"</strong> from the group?{' '}
